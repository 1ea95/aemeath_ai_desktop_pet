...
class AnimationManager:
    ...
    def ensure_music_frames(self) -> None:
        ...
        raw_frames, raw_delays = self._raw_gif_cache.get("Aemeath.gif", ([], []))
        if not raw_frames:
            raw_frames, raw_delays = load_gif_frames_raw("Aemeath.gif")
            if self._raw_gif_cache_enabled:
                self._raw_gif_cache["Aemeath.gif"] = (raw_frames, raw_delays)
    ...
    def preload_raw_gifs(self) -> None:
        ...
        if not self._raw_gif_cache_enabled:
            return
        if "Aemeath.gif" not in self._raw_gif_cache:
            raw_frames, raw_delays = load_gif_frames_raw("Aemeath.gif")
            self._raw_gif_cache["Aemeath.gif"] = (raw_frames, raw_delays)

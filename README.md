...
<p align="center">
  <img src="assets/gifs/idle4.gif" alt="Aemeath 桌面宠物" width="50%">
</p>
...
**GitHub仓库**：https://github.com/1ea95/aemeath_ai_desktop_pet
...
### 🎤 语音交互系统
- 🎯 **语音唤醒** - 基于sherpa-onnx的离线关键词检测，无需网络连接
- 🎧 **语音识别** - 阿里云ASR高精度语音转文字，支持实时流式识别
- 🔊 **语音合成** - 阿里云TTS自然语音合成，多种音色可选
- 🤖 **智能对话** - 语音识别结果自动转换为AI对话，支持多轮交互
- 🔄 **自动重连** - 网络异常时自动重试机制，确保服务稳定
- 🛠️ **语音命令修复系统** - 命令执行失败时自动弹出修复对话框，支持路径修改和命令删除
- 📝 **自定义命令支持** - 支持添加和管理自定义应用程序启动命令，动态加入LLM提示词
- 🎯 **智能命令识别** - 优化"动作+目标"组合命令的识别和处理，提高识别准确率

### 🤖 AI对话功能
- 💬 **多服务商支持** - DeepSeek/OpenAI/千问/智谱GLM/Kimi/豆包/自定义API
- 🌊 **流式回复** - 实时显示AI回复过程，提升交互体验
- 📝 **自定义模型** - 支持手动添加第三方API模型，灵活扩展
- 🔐 **配置保存** - API配置保存在用户目录，重启不丢失
- 🎭 **现代化UI** - 采用选项卡设计，LLM/ASR/TTS配置分离管理

### 🌐 划词翻译功能
- 📝 **划词复制** - 选中任意文本自动复制到剪贴板，智能过滤系统窗口
- ⌨️ **Ctrl长按翻译** - 按住Ctrl键超过0.5秒后松开触发翻译面板
- 🤖 **AI翻译** - 使用已配置的AI API进行翻译，支持多语言互译
- 📋 **无感复制** - 复制后自动恢复原剪贴板内容，不影响正常使用
- 📌 **置顶切换** - 支持置顶/取消置顶，灵活调整窗口层级
- 🌍 **多语言支持** - 支持简体中文、English、日本語等多种语言

### 🐾 智能运动系统
- 🧠 **状态机控制** - 游荡/跟随/好奇/休息，模拟真实宠物行为
- 🎭 **三种行为模式** - 安静模式、活泼模式、粘人模式，适应不同使用场景
- 🖱️ **鼠标跟随** - 智能距离判断，远距离主动跟随，近距离保持距离
- 💤 **随机休息** - 随机停下休息，模拟真实作息
- 🌀 **惯性移动** - 流畅自然的运动轨迹，支持拖拽后的惯性效果

### 🍅 生产力功能
- 🍅 **番茄钟** - 内置番茄工作法计时器，支持自定义时长和休息时间
- ⏰ **作息提醒** - 定期提醒喝水、休息、保持坐姿，关爱健康
- ⚡ **快速启动** - 支持自定义应用程序快速启动，与语音命令系统集成
- 🎤 **语音命令** - 通过语音快速启动应用程序、控制系统操作，提高工作效率
- 🛠️ **命令修复** - 语音命令执行失败时自动修复，减少手动配置时间

### 🎵 多媒体功能
- 🎵 **背景音乐** - 支持播放背景音乐，自动连播，支持多种音频格式
- 💬 **对话气泡** - 点击宠物显示问候语和互动，支持自定义内容
- 🎬 **多态动画** - 移动、待机、拖动等多种动画状态，流畅自然

### 🎨 界面定制
- 📏 **自由缩放** - 9档缩放（0.3x ~ 1.9x，步进为0.2x），适应不同屏幕尺寸
- 👻 **透明度调节** - 8档透明度（30% ~ 100%，步进为10%），融入桌面环境
- 👆 **鼠标穿透** - 默认开启，不遮挡日常操作，可随时切换
- 🖼️ **安静模式动画** - 点击随机触发特殊动画，增加互动趣味性

### ⚙️ 系统集成
- 🔄 **开机自启** - 支持开机自动运行，一键设置
- 🖥️ **系统托盘** - 完整的右键菜单控制，功能齐全
- ⌨️ **全局快捷键** - 支持快捷键快速操作，提高使用效率
- 🎯 **高DPI适配** - 良好适配高分屏，显示清晰锐利
- 📦 **即开即用** - 提供打包好的exe版本，无需安装环境
...
7. **语音命令修复** - 当语音命令执行失败时，系统会自动弹出修复对话框，允许用户修改命令路径或删除命令
...
AI配置界面于v3.0.1版本经过全面优化，采用三个独立选项卡设计，提供更直观、更高效的配置体验：
...
2. **ASR配置** - 语音识别相关配置
   ...
   - Token自动获取和状态显示
   - **v3.0.1版本新增**：Token自动更新管理系统集成，支持一键获取和自动刷新
...
**v3.0.1版本界面优化亮点**：
- 📑 **选项卡设计**：将LLM、ASR、TTS配置分离为三个独立选项卡，结构更清晰
- 🎯 **一键操作**：添加"自动获取"按钮，一键获取ASR Token，简化配置流程
- 📱 **响应式布局**：优化界面布局，支持滚动浏览，适应不同屏幕尺寸
- 🎭 **视觉升级**：采用现代化UI设计，色彩搭配更协调，操作更直观
- 🔍 **状态反馈**：实时显示配置状态和连接测试结果，提供即时反馈

支持以下AI服务商：

**说明**：表格中的“自定义”选项用于接入其他兼容 OpenAI 等格式的第三方 API 接口，而非特指某家服务商。
...
#### 功能特点
- **智能检测**：自动识别并跳过控制台、IDE等系统级窗口，防止干扰正常工作
- **无感复制**：复制后自动恢复原剪贴板内容，不影响正常使用
- **置顶功能**：点击📌按钮可切换窗口置顶状态
...
- **自定义训练**（高级功能）：支持高级用户使用自己的音频数据训练唤醒词，需要自行准备数据并处理模型转换
...
> 💡 **提示**：以下仅列出v3.0版本的主要升级亮点，详细功能介绍请参见上方「特色功能」章节。
...
- 🔄 **自动重连** - 网络异常时自动重试机制
- 🛠️ **语音命令修复系统** - 命令执行失败时自动弹出修复对话框
- 📝 **自定义命令支持** - 支持添加和管理自定义应用程序启动命令
- 🎯 **智能命令识别** - 优化"动作+目标"组合命令的识别和处理
...
1. **克隆仓库**
   ```bash
   git clone https://github.com/1ea95/aemeath_ai_desktop_pet.git
...
5. 提交Pull Request至https://github.com/1ea95/aemeath_ai_desktop_pet
...
### v3.0.4 (当前最新)
- 🛠️ **语音命令修复系统** - 全新实现语音命令执行失败时的自动修复机制
  - 🔧 智能错误检测：自动识别命令执行失败的原因
  - 🎯 修复对话框：提供路径修改、命令删除等多种修复选项
  - 📂 文件浏览器：集成文件选择器，方便用户定位可执行文件
  - 🔄 自动重试：修复后自动重新执行命令，验证修复效果
- 🎯 **自定义命令系统优化** - 完善自定义命令的识别和执行流程
  - 📝 动态命令提示：自动将自定义命令加入LLM提示词，提高识别率
  - 🎭 智能分类：保持原有命令分类结构，将自定义命令归类到"应用程序"
  - 🔍 精准匹配：优化"动作+目标"组合命令的识别和处理逻辑
  - ⚡ 缓存优化：实现命令提示词缓存，减少LLM调用延迟
- 🎨 **用户界面改进** - 优化命令相关对话框的布局和交互
  - 📐 布局优化：调整修复对话框的按钮位置和排列顺序
  - 🎯 用户体验：将浏览按钮移至左侧，符合常见UI设计规范
  - 🛠️ 代码质量：修复对话框中的缩进错误，提高代码可维护性

### v3.0.3
- 🐛 **Bug修复** - 修复AI对话相关问题和UI跟随问题
  - #006: 修复快捷菜单中打开AI对话无法正常回答的问题
  - #007: 修复托盘菜单调用AI对话时窗口类型错误的问题
  - #008: 修复快捷栏中的AI对话启动时无法正常跟随的问题
- 🔧 **技术改进** - 优化AI聊天面板的位置管理和UI组件尺寸同步
  - 改进AI聊天面板的初始化和位置计算逻辑
  - 确保UI管理器中的组件尺寸与实际窗口尺寸一致
  - 在窗口创建时立即设置正确的初始位置
...
- 🎉 **UI组件自动管理系统** - 全新实现UI组件自动管理器，解决UI元素相互遮挡问题
  ...
  - 📊 状态同步：隐藏桌宠时所有UI组件一起隐藏，保持界面一致性
  - 🎯 位置优化：音乐播放器显示时，番茄钟正确显示在歌名气泡上方
- 🔧 **技术架构优化** - 重构UI管理系统，提高代码可维护性
  - 📁 新增UI组件自动管理器（`src/ui/ui_manager.py`）
  - 🔗 统一UI组件注册和管理机制
  - 🛠️ 优化拖拽处理逻辑，确保缩放时UI组件正确跟随
  - 🎨 改进UI组件位置计算算法，提供更好的视觉体验
- 🐛 **Bug修复** - 修复多个UI相关问题
  - #001: UI元素相互遮挡问题
  - #002: UI组件缩放时位置不跟随中心改变
  - #003: 隐藏桌宠时音乐播放器UI没有跟着隐藏
  - #004: 配置列表中的不透明度错写成透明度
  - #005: 系统托盘菜单状态不同步

### v3.0.1
- 🔐 **ASR Token自动更新管理系统** - 实现阿里云ASR Token的自动获取、刷新和管理
  - 🔄 自动刷新机制：Token即将过期前自动刷新，确保语音识别服务不中断
  - 🛡️ 安全存储：Token安全存储在配置文件中，支持跨会话持久化
  - 📊 状态监控：实时显示Token状态和有效期，提供手动刷新选项
  - ⚙️ 智能配置：支持阿里云访问凭证配置，自动管理Token生命周期
- 🎨 **AI配置界面优化** - 重构配置对话框，提升用户体验
  - 📑 选项卡设计：将LLM、ASR、TTS配置分离为三个独立选项卡，结构更清晰
  - 🎯 一键操作：添加"自动获取"按钮，一键获取ASR Token，简化配置流程
  - 📱 响应式布局：优化界面布局，支持滚动浏览，适应不同屏幕尺寸
  - 🎭 视觉升级：采用现代化UI设计，色彩搭配更协调，操作更直观
- 🐛 **问题修复** - 修复Token管理相关的问题，提高系统稳定性
